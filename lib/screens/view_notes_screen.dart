...
class _ViewNotesScreenState extends State<ViewNotesScreen> {
  ...
  Future<void> _fetchNotes() async {
    ...
    try {
      ...
      if (_selectedFilter == "My Notes") {
        fetchedNotes = await _notesService.fetchUserNotes(widget.userId);
      } else if (_selectedFilter == "Saved Notes") {
        fetchedNotes = await _notesService.fetchSavedNotes(widget.userId);
      } else if (_selectedFilter == "Public Notes") {
        fetchedNotes = await _notesService.fetchPublicNotes();
      } else {
        fetchedNotes = await _notesService.fetchAccessibleNotes(widget.userId);
      }
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        fetchedNotes = fetchedNotes.where((note) {
          ...
          return title.contains(lowerCaseQuery) ||
              ...
              tagStrings.any((tag) => tag.contains(lowerCaseQuery));
        }).toList();
      }
      if (mounted) {
      ...
      if (mounted) {
        setState(() {
          _errorMessage = "Error loading notes. Please try again.";
        });
      }
    } finally {
      if (mounted) {
  ...
  Future<void> _toggleSaveNote(String noteId, bool currentlySaved) async {
    ...
    try {
      final success = await _notesService.toggleSaveNote(
          widget.userId, noteId, isCurrentlySavedInState);
      if (success && mounted) {
        setState(() {
  ...
  Future<void> _toggleNoteVisibility(
    ...
    try {
      ...
      if (mounted) {
        _showMessage("Failed to update note visibility: $e", isError: true);
      }
    }
  }
                                                            ...
                                                            ElevatedButton.icon(
                                                                ...
                                                                icon: _processingNotes[noteId] ==
                                                                    ...
                                                                    : const Icon(
                                                                        ...
                                                                        size:
                                                                            18),
                                                                label: Text(_processingNotes[noteId] ==
                                                                        true
                                                                    ? "Loading File..."
                                                                    : "View Note"),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          final result = await Navigator.pushNamed(context, '/upload_note',
                          ...
                          child: Column(
                            ...
                            children: [
                              Icon(Icons.note_alt_outlined,
                                  size: 70,
                                  color: theme.hintColor), // Changed icon
                  ...
                  : SliverPadding(
                      ...
                      sliver: SliverList(
                        delegate: SliverChildBuilderDelegate(
                            ...
                            if (isAdIndex) {
                              if (noteIndex < _notes.length) {
                                return NativeAdWidget(
                                  key: ValueKey(
                                      'view_notes_ad_slot_$noteIndex'), // Unique key
                                  adUnitKey:
                                      'ADMOB_NATIVE_AD_VIEW_NOTES', // MODIFIED
                          ...
                          childCount: _notes.isEmpty
                              ? 0
                              : (_notes.length * 2 -
                                  (_notes.isNotEmpty ? 1 : 0)),
                        ),
                      ),
                    ),
            ...
            children: [
              ...
              if (note['file_name'] != null &&
                ...
                Padding(
                    padding: const EdgeInsets.only(top: 8.0),
                    child: Row(children: [
                      Icon(_getFileIcon(note['file_type']?.toString() ?? ''),
              ...
              Row(
                ...
                children: [
                  Consumer<NoteEngagementProvider>(
                      builder: (context, engagementProvider, _) {
                    final int viewCount =
                        engagementProvider.getViewCount(noteId);
                    final int likeCount =
                        engagementProvider.getLikeCount(noteId);
                    return Row(children: [
